...
def run(j):
    ...
    for pc in range(len(full_tr_pairs)):
        ratio_good = 1.0
        ratio_bad = 1.0
        ratio = 0.0
    ...
    for i in range(20):
        model.fit([full_tr_pairs[:, 0], full_tr_pairs[:, 1]], full_tr_y,
              ...
              batch_size = 16, nb_epoch = 5, verbose = 1, shuffle = False)
    
        pred_full = model.predict([full_tr_pairs[:, 0], full_tr_pairs[:, 1]])
        full_tr_acc = compute_accuracy(pred_full, full_tr_y)
    ...
    print('* Accuracy on training set: %0.2f%%' % (100 * full_tr_acc))
    print('* Accuracy on test set: %0.2f%%' % (100 * te_acc))

    file = open("Base_Ord/results" + str(j) + ".txt", "w")
